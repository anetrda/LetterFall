...
function parseOBJ(text) {
  ...
  function addVertex(vert) {
    ...
    //console.log(" obj Index Str ", objIndexStr)
      if (!objIndexStr) {
        return;
      }
...
async function main() {
  ...
  window.addEventListener("keydown", (e)=>{
      console.log(" key pressed ", e, " \n type of keycode ",typeof e.keyCode)
      if ( e.keyCode == 32){        
        run = !run
        if (run){
          update()
  ...
  let letterList = [];
  const response = await fetch('./assets/newAlphabet.obj');  
  const text = await response.text();
  const obj = parseOBJ(text);
  console.log(" what is in the OBJECT ", obj)
  const parts = obj.objList.map(({data, object}) => {
    console.log(" what is in objList. data", data)
  ...
  console.log(" what is in the parts ", parts)
  ...
  const extents = getGeometriesExtents(obj.objList);
